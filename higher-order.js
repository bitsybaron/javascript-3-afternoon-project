...
let orderTotals = orders.map(obj => obj['price'] * (obj['tax'] + 1));
...
let bobsTotal = purchases.filter(element => element['owner'] === 'Bob').map(element => element.price).reduce((accumulator, currentValue) => accumulator + currentValue);
